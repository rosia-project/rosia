...
3. **Node Instance Initialization**: The user's `__init__` method is called on each node. This is where nodes load models, allocate resources, and set initial STAT values. Nodes do not communicate during this step.

4. **Collect Output Port STAT**: The coordinator gathers the initial STAT (Safe To Advance To) from all output ports. STAT is a promise about future message timestamps that prevents nodes from waiting indefinitely.

5. **Propagate STAT Through Graph**: STAT values are recursively propagated from source nodes through port connections. Each downstream input port's safe-to-advance-to is updated based on upstream output port STATs.

6. **Update Port STATs**: The computed STAT values are pushed back to all nodes, so each node has the correct initial safe-to-advance-to on all its ports.
