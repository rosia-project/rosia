...
| `advance_to_STAT()`        | Processes events and reactions from queues up to STAT. For each `InputPortEvent`: sets port values, fires reactions, recomputes STAT, and sends STAT updates to affected output ports. Port values are retained (not cleared) so that each port always holds its most recent value. Returns early if a `ShutdownEvent` is encountered or if advancing would exceed STAT. For eager reactions, the STAT bound is inclusive ($t$ <= STAT). |
