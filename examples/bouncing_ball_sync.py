"""3D bouncing balls synchronized into a single renderer.

Pipeline:
  Timer -> BallSimulator1 (fast) -+-> SyncRenderer
        -> BallSimulator2 (slow) -+

Both balls feed into one renderer with two input ports. The renderer
reacts to both ports, so Rosia's synchronization guarantees that both
ball positions are rendered at the same logical time — even though
BallSimulator2 is slow and lags behind in physical time.
...
@Node
class SyncRenderer:
    """Renders both balls together, synchronized at the same logical time."""
    ...
    def render(self):
        log.info(f"ball1 {self.ball1}  ball2 {self.ball2}")
